...
_.typeOf = function(anything){
    if(Array.isArray(anything)){
        ...
        return 'null';
    }else if(anything === Boolean){
        return 'boolean';
    }else if (anything === Object){
        return 'object';
...
_.first= function(array, number){
    if (number === undefined || typeof(number) !== 'number'){
        ...
        return array;
    }else if (number > 0 && array.length> number){
        return array.slice(0,number);
    }else if (number < 0 || Array.isArray(array) !== 'array'){
        return [];
    }
};
...
_.partition= function(array, func){
    ...
    arrTotal.push(arrTrue,arrFalse);
    return arrTotal;
};
...
_.map = function(collection, func){
    let arrFunc=[];
        // _.map=(collection,array) =>{
        //_.each(collection, (e, i, a) =>
        //arrFunc.push(func(e,i,a);
        for(let i = 0; i<collection.length; i++){
            if(Array.isArray(collection[i]) === true){
                arrFunc.push(func(collection[i], i, collection));
            }
        }
       for(let key in collection){
                    arrFunc.push(func(collection[key], key, collection));
  }
  return arrFunc;
};
...
_.every= function(collection, func){
    if(func === undefined){
        ...
        //Run the elements through the func and if false, return false
        if(Array.isArray(collection)=== 'array'){
            for(let i=0; i<collection.length; i++){
...
_.extend= function(object1, object2){
    var argumentsTotal = Array.prototype.slice.call(arguments); //Turns all of the arguments into an array
    var firstObj = argumentsTotal[0];
    var restObj = argumentsTotal.slice(1);
    for(let i=0; i< restObj.length; i++){
        for(let key in restObj[i]){
            firstObj[key]= restObj[i][key];
        }
    }
    return firstObj;
};  
 /*If the key doesnt exist, push the key and value from obj 2 to obj 1
